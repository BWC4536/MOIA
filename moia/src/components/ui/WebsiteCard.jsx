...
function StackBadge({ tech }) {
  ...
  return (
    <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border border-white/10 ${cls}`}>
      {tech}
    </span>
  )
}

/** Tarjeta rica para tipo: 'web' */
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {funcion && (
          <p className="text-xs text-zinc-500 mb-3 leading-relaxed">{funcion}</p>
        )}
        {!funcion && descripcion && (
          <p className="text-xs text-zinc-500 mb-3 leading-relaxed line-clamp-2">{descripcion}</p>
        )}
